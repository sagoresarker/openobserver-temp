# Sherlock Frontend UI Guidelines
...
Sherlock uses a comprehensive color system defined in `src/styles/_variables.scss` with support for both light and dark modes.
...
A **Platform UI** mode provides a simplified layout and a distinct visual identity aimed at devs and platform engineers. When enabled:

- **Sidebar:** Shows only **Logging**, **Dashboard**, **Metrics**, and **Alert** (no Home, Traces, RUM, Streams, Ingestion, IAM, etc.).
- **Visual design:** Slate/neutral base with a blue accent; `body.platform-ui` overrides `--o2-*` semantics with `--platform-*` tokens (see `src/styles/_variables.scss`).
- **Drawer:** Wider (180px) with labels visible; menu uses platform palette.

**Default:** Platform UI is **on by default** (simplified nav and platform palette).

**How to turn it off**

- **Build-time:** Set `VITE_PLATFORM_UI=false` in your `.env` or build environment to use the full sidebar and original styling.
- **Runtime:** The backend config can override via `zoConfig.simplified_nav === true` or `zoConfig.platform_ui === true` (frontend reads this from the config API).

When Platform UI is on, the frontend adds `platform-ui` to `document.body.classList` and uses the platform tokens for shell, cards, and menu. Logging, Dashboard, Metrics, and Alert screens inherit these tokens via existing `--o2-*` and `--q-primary` usage.
...
**Remember:** These guidelines exist to maintain consistency, quality, and performance across the Sherlock frontend. When in doubt, refer to existing components in the codebase that follow these patterns.
